...
def generate_mask():
    os.makedirs("./data/task02_train/mask", exist_ok=True)
    with open("./data/task02_train/labels.json", "r") as json_file:
    ...
    for idx in tqdm.tqdm(range(data_num), total = data_num):
        ...
        np.array(img)
        img.save(os.path.join("./data/task02_train/mask", file_path.split('.')[0]+'.jpg'))